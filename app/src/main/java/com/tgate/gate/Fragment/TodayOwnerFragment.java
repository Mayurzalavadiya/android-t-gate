...
public class TodayOwnerFragment extends Fragment {
    RecyclerView rvVisitorList;
    //OWNER
    ArrayList<VisitordataResponse.Datum> getVisitorOwnerList;
    visitorOwnerdataAdapter visitorOwnerdataAdapter;
    int currentpage = 1;
    boolean isIndex1;

    EndlessRecyclerOnScrollListener endlessRecyclerOnScrollListener;

    public TodayOwnerFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_today_owner, container, false);
        ...
        visitorOwnerdataAdapter = new visitorOwnerdataAdapter(getVisitorOwnerList, getContext());
        rvVisitorList.setAdapter(visitorOwnerdataAdapter);
        hostList(1);
...
//            Utility.showCustomProgressDialog(getActivity());
            RestClient.getInstance().getApiInterface().visitor_data_response("2","TODAY", page,  PrefsManager.readStringPrefsVal(PrefsManager.IS_LOGIN)).enqueue(new Callback<VisitordataResponse>() {
                @Override
                    ...
                    if (response.body() != null) {
                        if (response.body().getStatus() != null && response.body().getStatus().equals(true)) {
                            ...
                            if (b != null && b.size() > 0) {
                                ...
                                if (page == 1)
                                    visitorOwnerdataAdapter.replaceAll(b);
                                else
                                    visitorOwnerdataAdapter.addAllS(b);
