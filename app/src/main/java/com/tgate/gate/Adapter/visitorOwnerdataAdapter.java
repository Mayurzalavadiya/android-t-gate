...
public class visitorOwnerdataAdapter extends LoadMoreRecyclerAdapter<visitorOwnerdataAdapter.VisitorOwnerdataResponse, VisitordataResponse.Datum> {
    Context context;


    public visitorOwnerdataAdapter(List<VisitordataResponse.Datum> data, Context context) {
        super(data);
        this.context = context;
    }

    public void addAllS(List<VisitordataResponse.Datum> response) {
        getList().addAll(response);
        notifyDataSetChanged();
    }

    public void replaceAll(List<VisitordataResponse.Datum> response) {
        getList().clear();
        getList().addAll(response);
        notifyDataSetChanged();
    }
    ...
    @Override
    public VisitorOwnerdataResponse onCreateHolder(ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_today_list, parent, false);
            return new VisitorOwnerdataResponse(view);
        }

    @Override
    public void onBindHolder(VisitorOwnerdataResponse holder, int position) {

        VisitordataResponse.Datum getVisitorList=getItem(position);
...
*/
    ...
    public class VisitorOwnerdataResponse extends BaseRecyclerAdapter<visitorOwnerdataAdapter.VisitorOwnerdataResponse, VisitordataResponse.Datum>.ViewHolder  {
        CircleImageView img_userimg;
        AppCompatTextView txt_name, txt_accept,txt_city, txt_person, txt_company;

        public VisitorOwnerdataResponse(@NonNull View itemView) {
            super(itemView);
